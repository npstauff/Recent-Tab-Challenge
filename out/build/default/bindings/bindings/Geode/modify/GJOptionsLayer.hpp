...
namespace geode::modifier {
	...
	struct ModifyDerive<Der, GJOptionsLayer> : ModifyBase<ModifyDerive<Der, GJOptionsLayer>> {
		...
		void apply() override {
			...
			GEODE_APPLY_MODIFY_FOR_FUNCTION(base::get() + 0x206350, Membercall, GJOptionsLayer, addToggleInternal, char const*, int, bool, char const*)
			GEODE_APPLY_MODIFY_FOR_FUNCTION(base::get() + 0x205f80, Membercall, GJOptionsLayer, init, int)
			GEODE_APPLY_MODIFY_FOR_FUNCTION(base::get() + 0x203b00, Thiscall, GJOptionsLayer, didToggleGV, gd::string)
		}
	};
}
